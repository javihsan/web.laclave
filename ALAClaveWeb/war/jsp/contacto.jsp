<%@include file="cabecera.jsp"%>

<div class="portal-decorator-left-lateral"></div>



<div id="content-detail">


<table cellspacing="0" cellpadding="0" width="98%" height="100%"
	border="0">
	<tr>
		<td align="right" class="contentOptions"></td>
	</tr>
	<tr height="100%">
		<td valign="top"><!-- <h1 class="cabecera">Contacto</h1> -->


		<table border="0" style="width: 661px; height: 232px">
			<tbody>
				<tr>
					<td>
					<div>&nbsp;&nbsp;
					<img
						src="images/imagen_contacto.jpg"
						border="0" alt="Imagen Contacto" width="300" height="226" />&nbsp;</div>
					</td>
					<td>&nbsp;&nbsp;&nbsp;<img
						src="images/logo_clave.jpg" border="0"
						alt="Logo La Clave" width="35" height="54" />
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					</td>
					<td>&nbsp;
					<p><strong>D. Jos&eacute; Mar&iacute;a Legaza
					(Director)</strong></p>
					<p><strong>C/ Real, 34 -&nbsp;2&ordm;</strong></p>
					<p><strong>28400 Collado Villalba (Madrid)</strong></p>
					<p><strong>Tel&eacute;fono y Fax: 91 850 43 58</strong></p>
					<p><strong>M&oacute;vil: 667 406 248</strong></p>
					<p><strong>e-mail: </strong><a href="mailto:clavelegaza@gmail.com"><strong>clavelegaza@gmail.com</strong></a></p>
					<p>&nbsp;</p>
					</td>
				</tr>
			</tbody>
		</table>


<%@include file="pie.jsp"%>